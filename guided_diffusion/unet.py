...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
    #    super(UNetModel_newpreview, self).__init__() 
     #   self.input_blocks = nn.Sequential(nn.Conv2d(1, 128, kernel_size=3, padding=1), nn.BatchNorm2d(128), nn.ReLU(inplace=True))
...
#        self.hwm = UNetModel_newpreview.HybridBlockWrapperModule(nn.Sequential(nn.Conv2d(128, 32, kernel_size=3, padding=1),nn.BatchNorm2d(32), nn.ReLU(inplace=True)))
        

        
#class HybridBlockWrapperModule(nn.Module):
#    def __init__(self, blocks):
#        super(UNetModel_newpreview.HybridBlockWrapperModule, self).__init__()
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, hs = None):
        ...
        #                                zip(list(self.upscale_logits_ops)[::-1], anch_outputs[:-1][::-1])]),seg_outputs[-1]
            ...
            return tuple([i(j) if j is not None else None for i, j in 
                                        zip(self.unet.deconvs, self.unet.up_paths)] + [self.unet.last_conv(x)])
