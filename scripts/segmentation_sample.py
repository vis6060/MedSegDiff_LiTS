...
#    model.input_blocks[0][0].weight.data = th.zeros((128, 1, 3, 3))
#    model.hwm.conv_blocks_context[0].blocks[0].conv.weight.data = th.zeros((32, 0, 3, 3))
...
#    all_images = []
    ...
    # Load the checkpoint
    #    checkpoint = th.load('C:/Users/vis60/OneDrive/Desktop/SharingLaptoPC/DLProj/MedSegDiff/MedSegDiffCT/results/savedmodel100000.pt')
      #  print(checkpoint.keys()) 
# Create an instance of the model
     #   model = UNetModel_newpreview(image_size, in_channels, model_channels, out_channels, num_res_blocks, attention_resolutions, dropout, channel_mult, conv_resample, dims, num_classes, use_checkpoint, use_fp16, num_heads, num_head_channels, num_heads_upsample, use_scale_shift_norm, resblock_updown, use_new_attention_order, high_way)

# Update the model's state_dict to match the checkpoint's keys
...
#    new_state_dict.input_blocks[0][0].weight.data = th.zeros((128, 1, 3, 3))
#    new_state_dict.hwm.conv_blocks_context[0].blocks[0].conv.weight.data = th.zeros((32, 0, 3, 3))
    
    # Fix parameter mismatches
    if "input_blocks.0.0.weight" in new_state_dict:
            new_state_dict["input_blocks.0.0.weight"] = new_state_dict["input_blocks.0.0.weight"][:, 0:1, :, :]
    if "hwm.conv_blocks_context.0.blocks.0.conv.weight" in new_state_dict:
            new_state_dict["hwm.conv_blocks_context.0.blocks.0.conv.weight"] = new_state_dict["hwm.conv_blocks_context.0.blocks.0.conv.weight"][:, 0:0, :, :]
